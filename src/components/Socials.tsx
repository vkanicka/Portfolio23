...
const SocialsComponent = ({ socialsCollection }: Props) => {
  return (
    <address className="grid-start-10 grid-end-12 col-span-2 lg:ml-16">
      <ul className="flex flex-col items-center gap-0 rounded-lg bg-neutral-900">
        {socialsCollection?.map((social: Social, socialKey: number) => {
          return (
            <li
              className="w-full border-b border-solid border-neutral-800 px-2 py-5 text-center first:rounded-t-lg last:rounded-b-lg last:border-0 hover:bg-neutral-800"
              key={socialKey}
            >
              <a
